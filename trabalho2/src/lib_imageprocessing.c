...
imagem abrir_imagemCOMBRILHO_Processos(char *nome_do_arquivo, float brilho){
	...
	float* r = (float*) mmap(NULL, sizeof(float)*x*y, protection, visibility, 0, 0);
	float* g = (float*) mmap(NULL, sizeof(float)*x*y, protection, visibility, 0, 0);
	float* b = (float*) mmap(NULL, sizeof(float)*x*y, protection, visibility, 0, 0);
	...
	for(int k = 1; k<=8; k++){ 
		...
		{
			int inicio = (k-1)*x/7.99999f;
			int fim = (k)*x/7.99999f;			
			RGBQUAD color;
	...
	munmap (r, sizeof(float)*x*y);
	munmap (g, sizeof(float)*x*y);
	munmap (b, sizeof(float)*x*y);

	gettimeofday(&rt3, NULL);

	timersub(&rt3, &rt2, &drt2);  
	tempo1 = drt2.tv_sec + 0.000001*drt2.tv_usec;
	printf("\nTEMPO Usando Processos: %f segundos\n\n", tempo1);
  ...
  int fim = (vargp[0])*x/7.99999f;
   ...
   for (int j=0; j <y; j++) {
    ...
    if(vargp[1] == 8){
	  ...
	  timersub(&rt3, &rt2, &drt2);  
	  tempo1 = drt2.tv_sec + 0.000001*drt2.tv_usec;
	  printf("\nTEMPO Usando THREADS: %f segundos\n\n", tempo1);
